...
LTX-Video includes dedicated upscaler models that operate in latent space to enhance video resolution and frame count. The recommended workflow is a multi-step pipeline: generate at lower resolution, upscale in latent space, then denoise for final quality.
...
## Spatial Upscaler Pipeline

The spatial upscaler doubles the resolution of generated video in latent space, which is much faster than generating at full resolution directly.
...
latents = pipe(
    ...
    output_type="latent",  # Important: output latents, not pixels
).frames
...
# Step 3: Denoise upscaled latents (recommended)
final_video = pipe(
    conditions=[condition],
    prompt=prompt,
    width=ds_h * 2,
    height=ds_w * 2,
    num_frames=96,
    denoise_strength=0.4,       # Low denoise for refinement
    num_inference_steps=10,     # Fewer steps for refinement
...
1. The base generation at lower resolution is faster and uses less VRAM
2. The latent upscaler adds spatial detail efficiently
3. The denoising pass at full resolution cleans up artifacts from upscaling
4. The final resize step ensures exact target dimensions
...
The temporal upscaler increases the number of frames (frame interpolation in latent space):
...
For maximum quality, both upscalers can be applied in sequence:
...
- **Diffusers Pipeline Docs:** https://huggingface.co/docs/diffusers/main/en/api/pipelines/ltx_video
